...
app.get("/:id/books", (req, res) => {
  ...
  if (type && type === "count") res.json({ count: projects.length });
...
app.get("/:id/chapters", async (req, res) => {
  ...
  if (type && type === "count") res.json({ count: chapters.length });
  else res.json({ data: chapters });
});
...
const structureElementals = (elementals: ElementalJSON[]): Elemental[] => {
  ...
  return Object.values(elementalMap).filter(
    (elemental) => !elementals.find((e) => e.id === elemental.id)?.elementalID
  );
};
...
app.get("/:id/elementals", (req, res) => {
  ...
  const projectID = req.params.id;
  const elementals = ElementalStore.filter(
    (elemental) => elemental.projectID === projectID
  );

  if (type && type === "count") res.json({ count: elementals.length });
  else if (structure && structure === "true")
    res.json({ data: structureElementals(elementals) });
  else res.json({ data: elementals });
});
